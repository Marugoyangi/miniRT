...
/*   Updated: 2024/01/17 07:10:04 by jeongbpa         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	anti_aliasing(t_minirt *minirt, int x, int y, t_color *pixel_color)
{
	...
	while (i < minirt->camera.samples_per_pixel)
	{
		...
		*pixel_color = vec_add(*pixel_color, ray_color(&tmp, \
		minirt->objects, minirt->camera.max_depth));
		i++;
	}
}
...
void	print_color(t_minirt *minirt)
{
	...
	{
		x = 0;
		printf("\rRendering %d/%d", y + 1, minirt->img_height);
		fflush(stdout);
